...
\lstset{language=Lisp,
  ...
  % backgroundcolor=\color{gray} % AWEFUL!!
}
...
Together with tables, plots are the most commonly used viewer for Data Science use cases. Clerk comes with built-in support for the popular \href{https://github.com/vega/vega-embed}{vega} and \href{https://plotly.com/javascript/}{Plotly} plotting grammars.

In the following figure, the same \passthrough{\lstinline!iris!} dataset, as shown in the above table example, is used to render an interactive \passthrough{\lstinline!vega-lite!} plot using the \passthrough{\lstinline!clerk/vl!} viewer:
...
Then, a \passthrough{\lstinline!sparkline!} function is defined to generate graphs (using \passthrough{\lstinline!clerk/vl!}) that will be embedded into each table row later:
...
\includegraphics{images/result-anon-expr-5dsz8rmJVZqJQgRXAFtQ22xFSEXAKt.png}
